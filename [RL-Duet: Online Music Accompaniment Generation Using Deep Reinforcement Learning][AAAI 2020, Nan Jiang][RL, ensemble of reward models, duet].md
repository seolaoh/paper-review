...
### 2) Representation
* MIDI 데이터 사용, time을 16개의 note로 나눔, 각 pitch마다 별도의 또는 통일된 "hold" 기호 사용, "rest" 기호 사용
...
* 학습 과정은 2개의 phase를 반복: generation phase, reward phase
* generation phase가 모두 끝나고 전체 음악이 다 연주되면, reward agent가 각 time-step의 reward를 계산하여 전체 Rt를 계산하고, 이 값을 이용해서 generation agent의 action policy π(at|st)와 reward function V(st) 업데이트
...
* machine 부분의 horizontal 시간적 일관성과 human part와 machine part 사이의 vertical 조화 관계를 동시에 고려하도록
  * Joint Modeling: 직접적으로 p(mt|st) 학습 (a) / pre-context와 post-context를 동시에 고려하여 학습 (b)
  * Horizontal View: intra-part의 시간적 일관성을 위해 오직 machine part의 pre-context와 post-context 고려, 많은 monophonic music을 학습에 이용 가능 (c)
  * Vertical View: inter-part harmony를 capture, 오직 전체 human part와 machine part의 중앙 excerpt의 조화를 모델링 (d)
